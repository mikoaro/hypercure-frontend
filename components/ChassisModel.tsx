...
export default function ChassisScene({ currentTemp }: { currentTemp: number }) {
  return (
    <div className="h-125 w-full bg-[#0a0e17] rounded-xl border border-white/10 overflow-hidden relative">
      <div className="absolute top-4 left-4 z-10 pointer-events-none">
        <div className="text-[10px] text-[#00ff9d] font-mono mb-1">LIVE DIGITAL TWIN</div>
        <div className="text-white/50 text-xs uppercase tracking-widest">WebGL Thermal Shader</div>
      </div>

      <Canvas camera={{ position: [20, 8, 5], fov: 20 }}>
        <ambientLight intensity={0.5} />
        <pointLight position={[10, 10, 10]} intensity={1} />
        
        <ThermalAsset url="/assets/models/f1_front_wing.glb" tempC={currentTemp} position={[1, -0.5, -1]} rotation={[0, Math.PI, 0]} />
        {/* <ThermalAsset url="/assets/models/f1_chassis.glb" tempC={currentTemp} position={[0, -0.5, -3.0]} rotation={[0, Math.PI, 0]} /> */}

        <Grid position={[0, -0.5, 0]} args={[40, 40]} cellColor="#ffffff" sectionColor="#00ff9d" fadeDistance={25} opacity={0.15} />
